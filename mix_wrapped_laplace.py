...
def em_wrap_mix_laplace(x,mu=pi,beta=1.0,theta=0.9,eps=0.0001,max_iter=1000,L=20,wrap=2*pi,conv_lik=False,verbose=False):
  ...
  while diff > eps and iter_num < max_iter:
    ...
    z = normalize(np.insert(theta_old[0]*laplace.pdf(np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1)),\
    ...
    mu = weighted_median((np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1))).flatten('F'),z1.flatten('F'),sort=False)
    # Update beta
    beta = np.multiply(abs(((np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1)))-mu)),z1).sum() / norm_const
    mu %= (2*pi)
