...
def em_wrap_mix(x,mu=3.14,sigma2=1,theta=0.9,eps=0.0001,max_iter=1000,L=20,wrap=2*pi,verbose=False):
  ...
  while diff>eps and iter_num<max_iter:
    ...
    z = normalize(np.insert(theta_old[0]*norm.pdf(np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1)),\
    ...
    mu = np.multiply(np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1)),z1).sum()/norm_const
    # Update sigma
    sigma2 = np.multiply((((np.transpose(np.tile(x,(2*L+1,1)))+2*pi*np.tile(list(range(-L,L+1)),(len(x),1)))-mu)**2),z1).sum()/norm_const
    mu %= (2*pi)
