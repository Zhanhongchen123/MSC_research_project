    ...
    for c in range(n_chains):
        ...
        for i in range(1,burnin+n_samp):
            ...
            for k in range(N // 20):
                ...
                probs = np.array([(h1 + h2) * h3 for h1, h2, h3 in zip(Nah,[gamma0,delta0],[prob_human,prob_auto])],dtype=float)
                probs /= sum(probs)
                ha = np.random.choice(range(2),p=probs)
                kappa_probs /= sum(kappa_probs)
                z[j] = L+1 if ha==0 else np.random.choice(np.arange(-L,L+1),p=kappa_probs)
                ## Update the counts
                zsamp = z[j]
                if zsamp != zold:
                    Na[zold+L] -= 1
            ...
            if laplace:
                ...
                mu_prop = float(np.random.laplace(loc=mu[i-1,c],scale=0.1,size=1))
